...
class CameraCalibrator(object):
    ...
    def calibration(self, corner_height:int, corner_width:int, square_size:float):
        file_names = glob.glob('../img/*.JPEG') + glob.glob('../img/*.jpg') + glob.glob('../img/*.png') + glob.glob('./img/*.jpeg')
        objs_corner = []
        imgs_corner = []
    ...
    def cut_video(self, video_path):
        ...
        if not cap.isOpened():
            ...
            return False
        # 获取视频文件名
        video_name = cap.get(cv.CAP_PROP_FILENAME)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, choices=['calibrate', 'rectify', 'cut_video'], help='to calibrate or rectify')
    ...
    if args.mode == 'calibrate':
        ...
        if args.video_path:
            if os.path.exists(args.video_path):
                calibrator.cut_video(args.video_path)
            else:
                print("File {} does not exist.".format(args.video_path))
    else:
